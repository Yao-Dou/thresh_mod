...
export default {
    ...
    methods: {
        process_target_html() {
            try {
                this.process_target_html_with_selected_span(null);
            } catch (e) {
                this.target_html = ''
                console.warn(e)
            }
        ...
        process_target_html_with_selected_span(category) {
            ...
            const sent = hit.target;
            const highlightable_areas = hit.target_highlightable_areas;

            try {
                // If highlightable areas are defined and not empty, use the new renderer.
                if (highlightable_areas && highlightable_areas.length > 0) {
                     this.target_html = this.render_sentence_with_highlightable_areas(sent, highlightable_areas, category);
                } else {
        ...
        render_sentence_with_highlightable_areas(sentence, areas, category) {
            ...
            sorted_areas.forEach(([start, end]) => {
                ...
                const processed_sub_html = this.render_sentence(highlightable_text, 'output_idx', 'target_span', category, start); // pass offset `start`
        ...
        select_target_html(e) {
            ...
            if (highlightable_areas && highlightable_areas.length > 0) {
                ...
                let currentElement = document.querySelector('#target-sentence').firstChild;
        ...
        deselect_target_html(e) {
            ...
            if (selection.isCollapsed) {
                // No active selection, safe to regenerate
                this.process_target_html_with_selected_span(null);
            }
        }
    },
